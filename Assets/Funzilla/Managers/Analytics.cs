	...
	internal class Analytics : Singleton<Analytics>
	{
		...
		private static void LogFirebaseEvent(string eventName)
		{
			if (GameManager.FirebaseOk)
			{
				...
				{
					Debug.Log("Firebase analytics exception: " + e.ToString());
				}
			}
			else
			{
		...
		{
#if UNITY_EDITOR || DEBUG_ENABLED
			Debug.Log($"level_{Profile.Level}_start");
#else
		...
		{
#if UNITY_EDITOR || DEBUG_ENABLED
			Debug.Log($"level_{Profile.Level}_complete");
#else
		...
		{
#if UNITY_EDITOR || DEBUG_ENABLED
			Debug.Log($"level_{Profile.Level}_fail");
#else
